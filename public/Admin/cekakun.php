  ...
  <body class="bg-gray-100 font-family-inter flex">
    <aside class="relative bg-[#152A38] h-screen w-64 hidden sm:block shadow-xl">
      ...
      <nav class="text-white text-base font-semibold pt-0">
        ...
        <a href="datachart.php?id=<?= $_SESSION['id_user'] ?>&role=<?= $_SESSION['role'] ?>" class="flex items-center text-white opacity-75 hover:opacity-100 py-4 pl-6 nav-item">
          <i class="fas fa-chart-bar mr-3"></i>
          Data Chart
        </a>
        <a href="validasi.php?id=<?= $_SESSION['id_user'] ?>&role=<?= $_SESSION['role'] ?>" class="flex items-center text-white opacity-75 hover:opacity-100 py-4 pl-6 nav-item">
        ...
        <a href="user.php?id=<?= $_SESSION['id_user'] ?>&role=<?= $_SESSION['role'] ?>" class="flex items-center text-white opacity-75 hover:opacity-100 py-4 pl-6 nav-item">
          <i class="fas fa-user-cog mr-3"></i>
          Kelola User
        </a>
      </nav>
      <a href="../Login/logout.php?id=<?= $_SESSION['id_user'] ?>&role=<?= $_SESSION['role'] ?>" class="absolute w-full upgrade-btn bottom-0 active-nav-link text-white flex items-center justify-center py-4">
    ...
    <div class="w-full flex flex-col h-screen overflow-y-hidden">
      ...
      <header class="w-full items-center bg-white py-2 px-6 hidden sm:flex">
        ...
        <div x-data="{ isOpen: false }" class="relative w-1/2 flex justify-end">
          ...
          <div x-show="isOpen" class="absolute w-32 bg-white rounded-lg shadow-lg py-2 mt-16">
            <a href="cekakun.php?id=<?= $_SESSION['id_user'] ?>&role=<?= $_SESSION['role'] ?>?id=<?= $_SESSION['id_user'] ?>" class="block px-4 py-2 account-link hover:text-white">Account</a>
          </div>
        </div>
      </header>
      ...
      <header x-data="{ isOpen: false }" class="bg-[#152A38] w-full bg-sidebar py-5 px-6 sm:hidden">
        ...
        <nav :class="isOpen ? 'flex': 'hidden'" class="flex flex-col pt-4">
          ...
          <a href="datachart.php?id=<?= $_SESSION['id_user'] ?>&role=<?= $_SESSION['role'] ?>" class="flex items-center text-white opacity-75 hover:opacity-100 py-2 pl-4 nav-item">
            <i class="fas fa-chart-bar mr-3"></i>
            Data Chart
          </a>
          <a href="validasi.php?id=<?= $_SESSION['id_user'] ?>&role=<?= $_SESSION['role'] ?>" class="flex items-center text-white opacity-75 hover:opacity-100 py-2 pl-4 nav-item">
            <i class="fas fa-sticky-note mr-3"></i>
            Validasi Berkas
          </a>
          ...
          <a href="user.php?id=<?= $_SESSION['id_user'] ?>&role=<?= $_SESSION['role'] ?>" class="flex items-center text-white opacity-75 hover:opacity-100 py-2 pl-4 nav-item">
            <i class="fas fa-user-cog mr-3"></i>
            Kelola User
          </a>
                ...
                <div class="flex flex-wrap w-full mt-1 pl-1 mb-10">
                        ...
                        <div class="flex-1 p-3 py-5">
                            <div class="flex flex-col items-center text-center">
                                ...
                                <div class="my-10 border-2 border-gray-600 hover:bg-black  rounded-full">
                                    <label for="changephoto" class="cursor-pointer hover:text-white text-black p-10">Change Photo
